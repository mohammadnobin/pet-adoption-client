...
const AllPets = () => {
  ...
  <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
    <div className="relative w-full md:w-[500px]">
      ...
      <input
        ...
        className="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-black dark:border-white dark:placeholder-white dark:text-white dark:focus:white dark:focus:white"
      />
      <button
        type="submit"
  ...
  <div className="xl:w-7xl w-full mx-auto p-4">
    ...
    <div className="hidden md:block overflow-x-auto rounded-lg shadow-lg border border-gray-300 dark:border-white">
      <table className="min-w-full divide-y divide-gray-200 dark:divide-white">
        <thead className="bg-secondary text-white">
          <tr className="border-2 border-secondary dark:border-white">
        ...
        <tbody className="bg-white divide-y divide-gray-200 dark:bg-white/20 dark:divide-white">
          {isLoading ? (
            [...Array(6)].map((_, idx) => (
              <tr key={idx} className="border-2 dark:border-white border-secondary">
            ...
            <tr>
              <td colSpan={5} className="text-center py-6 text-gray-500 italic dark:text-gray-400">
                No pets found.
              </td>
            </tr>
          ) : (
            pets.map((pet, index) => (
              <tr
                key={pet._id || index}
                className="hover:bg-secondary/20 bg-white border-2 dark:border-white border-secondary mb-3 transition-colors duration-200 dark:bg-black dark:hover:bg-black/30"
              >
                <td className="px-6 py-4 text-center">
                  <img
                    src={pet.pteImage}
                    alt={pet.petName}
                    className="w-32 h-32 object-cover rounded-md mx-auto border border-gray-200 dark:border-gray-700"
                  />
                </td>
                <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{pet.petName}</td>
                <td className="px-6 py-4 dark:text-gray-300">{pet.petCategory}</td>
                <td className="px-6 py-4 text-center">
                  {/* {pet.adopted === "adopted" ? ( */}
                    <span className="inline-block px-3 capitalize py-1 rounded-full text-xs font-semibold border-2 border-secondary text-secondary dark:border-secondary dark:text-secondary">
                     {pet.adopted}
                    </span>
                  {/* ) : (
    ...
    <div className="md:hidden space-y-4">
      {isLoading ? (
        [...Array(3)].map((_, idx) => (
          <div
            key={idx}
            className="border border-gray-300 rounded-lg p-4 shadow dark:border-white dark:bg-black"
          >
        ...
        <p className="text-center py-6 text-gray-500 italic dark:text-gray-400">No pets found.</p>
      ) : (
        pets.map((pet, index) => (
          <div
            key={pet._id || index}
            className="border border-gray-300 rounded-lg p-4 shadow hover:shadow-lg transition dark:border-white dark:bg-black dark:hover:bg-white/30"
          >
            <div className="flex items-center space-x-4">
              <img
                src={pet.pteImage}
              ...
              />
              <div className="flex-1 space-y-2">
                <h3 className="text-lg font-semibold text-secondary dark:text-secondary">
                  {pet.petName}
                </h3>
                ...
                <p className="text-sm text-gray-700 dark:text-gray-300">
                  Status:{" "}
                  {pet.adopted === "adopted" ? (
                    <span className="inline-block px-2 py-1 rounded-full text-xs font-semibold border-2 border-secondary text-secondary dark:border-secondary dark:text-secondary">
                      Adopted
                    </span>
                  ) : (
