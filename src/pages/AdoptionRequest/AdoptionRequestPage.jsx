...
const AdoptionRequestPage = () => {
  ...
  return (
    ...
    //       কোনো pending adoption request নেই।
    //     </p>
    //   ) : (
  ...
  {isLoading ? (
    ...
    <>
        ...
        <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow">
          ...
          <tbody>
            {pets.map((pet) => (
              ...
              >
                <td className="py-3 px-4">{pet.petName}</td>
                <td className="py-3 px-4">{pet.adoptedHistory.adopterName}</td>
                <td className="py-3 px-4">{pet.adoptedHistory.adopterEmail}</td>
                <td className="py-3 px-4">{pet.adoptedHistory.adopterPhone}</td>
                <td className="py-3 px-4">{pet.adoptedHistory.adopterAddress}</td>
                <td className="py-3 px-4 flex justify-center gap-2">
                  <button
                    onClick={() => acceptMutation.mutate(pet._id)}
                    className="bg-green-500 cursor-pointer hover:bg-green-600 text-white px-3 py-1 rounded"
      ...
      <div className="md:hidden space-y-4">
        {pets.map((pet) => (
          <div key={pet._id} className="border rounded-lg p-4 shadow bg-white">
